...
var eb = function($) {
    ...
    function loadPosts(callback, previousPages) {
        ...
        fetchPages(pageNumbers, function($pages) {
            ...
            if (previousPages) {
                $existingPosts.prepend($posts).masonry("prepended", $posts).one("layoutComplete", callback);
    ...
    function toaster(message) {
        $("#toaster").remove();
        var html = '<div id="toaster" class="toaster">' + '<button type="button" class="toaster__close js-close-toaster">&times;</button>' + "<p>" + message + "</p>" + "</div>";
        $("#main").append(html);
        setTimeout(function() {
            closeToaster()
        }, 5e3)
